...
#endif
/* Maximum possible number of 1D terrain atlases. (worst case, each 1D atlas
 * only has 1 tile) */
...
CC_VAR extern struct _Atlas1DData {
  ...
  int Mask, Shift;
  /* Texture V coord that equals the size of one tile. (i.e.
   * 1/Atlas1D.TilesPerAtlas) */
...
#define Atlas2D_TileX(texLoc)                                                  \
  ((texLoc) & ATLAS2D_MASK) /* texLoc % ATLAS2D_TILES_PER_ROW */
#define Atlas2D_TileY(texLoc)                                                  \
  ((texLoc) >> ATLAS2D_SHIFT) /* texLoc / ATLAS2D_TILES_PER_ROW */
/* Returns the index of the given tile id within a 1D atlas */
#define Atlas1D_RowId(texLoc)                                                  \
  ((texLoc) & Atlas1D.Mask) /* texLoc % Atlas1D_TilesPerAtlas */
/* Returns the index of the 1D atlas within the array of 1D atlases that
 * contains the given tile id */
#define Atlas1D_Index(texLoc)                                                  \
  ((texLoc) >> Atlas1D.Shift) /* texLoc / Atlas1D_TilesPerAtlas */

/* Loads the given tile into a new separate texture. */
GfxResourceID Atlas2D_LoadTile(TextureLoc texLoc);
/* Attempts to change the terrain atlas. (bitmap containing textures for all
 * blocks) */
